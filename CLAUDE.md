...
### 2026-08-26 13:15 Asia/Taipei（最新）— 簽名檔內容欄位改成富文字編輯，貼上不再壓成純文字，新增文字大小／顏色格式
...
- 部署狀態：`index.html` 純前端，git push 後自動生效。**`worker/` 需要重新部署才會讓新的 20000 字元上限生效**（`cd worker && npx wrangler deploy`）——沒部署前，簽名檔內容欄位的富文字編輯／工具列（純前端功能）已經正常可用，只有「超過 10000～20000 字元之間的內容」這個邊界情況會被舊版 Worker 用舊的 10000 字元上限擋下，不影響絕大多數正常長度的簽名檔。
- commit：（見下方 push 紀錄）
