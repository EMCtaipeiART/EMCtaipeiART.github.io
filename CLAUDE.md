...
- 追查過程：
  1. **縮放把手是「復原」不是「新增」**：`CLAUDE.md` 2026-08-26 那則紀錄記載，這個系統一開始（2026-08-18）就有過一顆圖片右下角的縮放把手，但因為完全沒有圖示、單純一個看不出用途的深色小方塊，使用者誤以為是「選取框」而移除掉；當時的紀錄裡明確寫著「這次新增的拖曳排序把手／換行切換鈕都吸取同一個教訓，一律搭配清楚的 SVG 圖示＋title/aria-label，不會是一顆看不出用途的空白方塊」——這次重新加回縮放功能，直接沿用這個既有教訓，用對角縮放箭頭 SVG 圖示＋清楚的 title/aria-label，不重演同一個錯誤。
...
- 部署狀態：`index.html`、`CLAUDE.md` 純前端，git push 後自動生效。**`worker/` 需要手動部署才會生效**（`cd worker && npx wrangler deploy`）——沒部署前，附加檔案功能點下去會因為 Worker 還沒有 `attachments` 這個欄位的解析邏輯而被舊版忽略（不會報錯，只是附件不會真的寄出去，回退成沒有附件的普通信件）；圖片縮放與自由拖曳定位是純前端邏輯，push 後即生效，不受 Worker 部署進度影響。
- commit：（見下方 push 紀錄）
...
- 影響檔案：`design_dashboard.html`、`backend/test/backend.test.mjs`、`CLAUDE.md`。
...
- 風險區塊：
  - `openReplyMethodChooser` 目前只用既有的 `request.mail` 權限把關（跟改動前「回信」按鈕本身的權限判斷一致），沒有額外限制「填寫修改需求信」只能專案負責人用、「設計師回覆信」只能設計師用——這是刻意的範圍收斂：真要做角色級別的強制區分，需要在 `assets/access-control.js`／`worker/src/model.ts`／`backend/app.mjs` 三處新增權限類型（比照 CLAUDE.md 過去新增 `request.mail` 那次的規模），這次沒有做；但底層寫入動作本身仍然有既有的權限保護——`updateCaseRow()`（NAS 路徑寫入）與 `openStatusEditor()` 都已經要求 `isDesignerLogin()`，`media.manage` 權限則保護 NAS／上傳這一段，不是完全沒有保護，只是選單本身沒有先依角色隱藏選項。
...
- 部署狀態：`index.html`、`json_database_admin.html`、`CLAUDE.md` 純前端／文件，git push 後自動生效；**`worker/` 需要手動部署才會生效**（`cd worker && npx wrangler deploy`）——沒部署前，`request.edit`／`request.delete` 仍沿用舊版「案件層級文字比對」邏輯（`isCustomerOwnerAccount`＋`matchesOwnerIdentity`），不會套用新的客戶別白名單制；前台的部門／組別可見性過濾與灰階按鈕是純前端邏輯，push 後即生效，不受 Worker 部署進度影響。
- commit：（見下方 push 紀錄）
