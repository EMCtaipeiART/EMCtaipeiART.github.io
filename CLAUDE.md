...
### 2026-08-26 16:40 Asia/Taipei（最新）— 信件串瀏覽改成真正的淨化 HTML 渲染：簽名檔不再跑版、內嵌圖片回到原本位置、所有信件預設收合
...
- 部署狀態：`index.html`、`backend/test/backend.test.mjs` 純前端／測試檔案，git push 後自動生效。**`worker/` 需要手動部署才會生效**（`cd worker && npx wrangler deploy`）——沒部署前，`getCaseMailThread` 仍會用舊版單一簽名檔比對、不會回傳 `bodyHtml`／`images[].contentId`，前端會正確 fallback 到既有的純文字＋連結轉換路徑（不會報錯或顯示空白，只是這次要修的三個問題還沒真正生效），部署後才會完整套用新的多候選簽名檔比對與淨化 HTML 渲染。
- commit：（見下方 push 紀錄）

### 2026-08-26 15:20 Asia/Taipei（次新）— 修正「最新案件列表」最後一列被裁掉看不到；問題回報彈窗改成列表在上、欄位在下並修正圓角
