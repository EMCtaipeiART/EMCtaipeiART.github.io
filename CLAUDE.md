...
- 影響功能：
  ...
  3. **Worker 新增六個 action**，全部用 `this.requireAccess(database,session,'request.mail')` 把關（這是這次第一批真正檢查這個既有但從未被任何 action 用過的權限的地方，不用新增權限、不用動 `assets/access-control.js` 的權限目錄）：
     - `gmailOauthConnect`：PKCE 授權碼交換（`client_id` 沿用既有的 `GOOGLE_OAUTH_CLIENT_ID`，`client_secret` 用新的 `GMAIL_OAUTH_CLIENT_SECRET`，`redirect_uri` 沿用既有的 `ERP_REDIRECT_URI`，即 `https://emctaipeiart.github.io`，不用另外註冊新的重新導向 URI），成功後用 access_token 打 `userinfo` 拿 Gmail 地址，寫進 `gmail_tokens`。
