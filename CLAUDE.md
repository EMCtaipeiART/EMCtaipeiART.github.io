...
- 部署狀態：純前端，git push 後自動生效。**不需要重新部署 Worker 或任何 Apps Script**——完全重用既有已經上線的 `scheduleCaseMail`／`listScheduledMail`／`cancelScheduledMail` action。**這次尚未 commit／push**——本機工作目錄同時存在另一個並行工作階段留下、跟這次改動完全無關的未提交變更（`json_database_admin.html`／`backend/test/backend.test.mjs`／`index.html` 的另一部分，是客戶別「個別人員」可見性與刪除動作二次確認相關的功能），為了不把對方尚未確認完成的工作一起打包進這次的 commit，這次先只把改動內容驗證完成、寫進這份文件，commit 動作留待跟使用者確認後再進行。
- commit：（尚未提交，見上方部署狀態說明）
