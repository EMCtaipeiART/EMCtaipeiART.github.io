...
### 2026-08-18 18:20 Asia/Taipei（最新）— 刪除按鈕改配「未開始」膠囊色；Gmail 信件串彈窗僅顯示寄件人＋非寄件帳號隱藏回覆；Gmail 撰寫/回信編輯區修正 Chrome 點擊無法編輯與預設變粗體、插入圖片改用鎖定比例的自訂縮放把手、插入 NAS 路徑改用「過稿中」同一套資料夾選擇器視窗
