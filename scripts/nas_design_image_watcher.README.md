...
每張 NAS 圖片會依「案件／輪次／來源相對路徑／檔案版本」產生穩定防重鍵；
上傳 Apps Script 會用這個鍵沿用已建立的 Drive 檔案。因此即使第一次上傳其
實已成功、回應卻在途中逾時，背景排程再次送出同一批時仍會得到相同網址，
不會在修改紀錄裡再追加一整組相同圖片。NAS 端也會先保存「這批已送出、結
果待確認」狀態；下一輪先核對正式資料庫，五分鐘內不會因發布延遲立刻重送。
...
1. **完全不遞迴進任何子資料夾**：掃描只認案件指定資料夾「這一層」的檔案，
   底下不管有什麼子資料夾（舊版本、參考素材、Links、其他分類…）一律不會
   被掃到，不需要另外列出要排除哪些名稱。`ignoreFolderNames` 這個設定欄位
   跟 `isIgnoredFolderName()` 是遞迴掃描年代留下的舊機制，現在已經沒有實際
   作用（本來就不會進去任何子資料夾），保留只是避免多餘變動，之後如果要
   徹底清掉可以一併移除。
2. **檔名關鍵字（案件的「設計圖檔名關鍵字」欄位）**：設計師在 NAS 資料夾
   ...
   位留白的話，行為退回舊版（資料夾這一層底下所有檔案都算，子資料夾一律
   不掃描），選擇器畫面上留白按下確認時會另外跳一次警告，提醒可能誤抓其他
   案件的圖，但不會強制擋下（有些案件真的有專屬資料夾，不需要關鍵字也沒
   問題）。
...
| `ignoreFolderNames` | 舊機制殘留的設定欄位，掃描已經完全不遞迴進任何子資料夾，這個清單目前沒有實際作用，保留只是避免多餘變動 |

狀態快取存在 `scripts/nas_design_image_watcher.state/sync-state.json`
（已加進 `.gitignore`），記錄每個檔案的大小/修改時間、有沒有被歸類到某一
輪、目前抓到第幾輪；預覽圖存在同資料夾的 `previews/`。每個資料夾在某一輪
第一次成功上傳後會標記為已封存：同一來源檔之後再次儲存，或該資料夾之後才
出現的新檔案，都只更新本機待傳預覽，不會再追加到已完成的初稿／一修／二修；
等資料庫建立下一個修改輪次後，才會把最新版本歸到新輪次。一次掃描同時找到
的多張初稿仍會一起上傳，不受封存影響。刪掉整個
`nas_design_image_watcher.state/` 資料夾會讓下次執行把所有檔案當成
「新增」、所有輪次重新判斷一次（已經上傳過的圖片還是會被視為「待歸類」
再上傳一次，等於重推；正式 API 仍會以同一輪、同檔名只保留第一筆作為最後
一道防護）。
