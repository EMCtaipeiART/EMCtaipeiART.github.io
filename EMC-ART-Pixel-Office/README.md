...
設計需求系統會以 `EMC-ART-Pixel-Office/dist/?embed=1` 將場景嵌入「設計師專長與案件分配」區塊。這個模式隱藏頁首、編輯工具、名單與說明，但保留人物點選與人物資料卡；鍵盤移動與狀態編輯仍停用，並每 3 秒讀取同一份多人狀態。首頁維持原本左右欄與 iframe 的 3:2 外框，左右卡片底邊對齊；Canvas 在外框內裁切、放大中央座位區，桌機版為 137%、手機版為 167%，並保留下排姓名。人物卡最寬只佔 iframe 欄位的三分之一。完整遊戲仍使用不帶 `embed=1` 的網址。
