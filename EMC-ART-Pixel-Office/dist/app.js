...
// 嵌入模式：設計需求系統用 iframe 把場景放進「設計師專長與案件分配」當展示用。只顯示場景、不接受操作，
// 但狀態照樣同步，所以那邊看到的跟遊戲裡是同一個畫面。
...
game.addEventListener('pointerdown',e=>{if(!ready||embedMode)return;const rect=game.getBoundingClientRect(),x=(e.clientX-rect.left)*W/rect.width,y=(e.clientY-rect.top)*H/rect.height;const hit=[...hits].reverse().find(h=>x>=h.x&&x<=h.x+h.w&&y>=h.y&&y<=h.y+h.h);if(hit){if(hit.type==='photo')openPhoto(hit.i);else{select(hit.i);game.focus({preventScroll:true});}}else select(null);});
game.addEventListener('pointermove',e=>{if(embedMode)return;const r=game.getBoundingClientRect(),x=(e.clientX-r.left)*W/r.width,y=(e.clientY-r.top)*H/r.height;game.style.cursor=hits.some(h=>x>=h.x&&x<=h.x+h.w&&y>=h.y&&y<=h.y+h.h)?'pointer':'default';});
