...
/** 離席狀態：椅子與電腦都不畫，只留灰階空桌；狀態圖示放在桌面上，文字在圖示上方（不會被圖示或桌子遮住）。 */
function drawStatusMarker(s){const p=stationPerson(s);if(!p||p.status==='present')return;const status=statuses.find(item=>item.id===p.status),x=s.x,iconY=s.y+10;drawSymbol(ctx,status.symbol,x,iconY,62);ctx.save();ctx.font='700 13px -apple-system,BlinkMacSystemFont,"PingFang TC","Noto Sans TC",sans-serif';const w=Math.round(ctx.measureText(status.label).width+22),h=22,labelY=iconY-29-h-4;softPanel(ctx,x-w/2,labelY,w,h,{radius:11,fill:'#ffffff',stroke:'#c9d3e0',lineWidth:1,shadow:true});ctx.fillStyle='#273b50';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(status.label,x,labelY+h/2+.5);ctx.restore();hits.push({type:'status',i:names.indexOf(p.name),x:x-45,y:labelY,w:90,h:iconY+29-labelY});}
