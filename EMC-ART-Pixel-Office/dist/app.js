...
// overtime-filter.png：正面、側面、背面三格；黑眼圈對齊眼睛，側面向左時與人物一起鏡像。
function drawOvertimeFilter(context,dir,feetX,feetY,height=142){if(!overtimeSheet.complete||!overtimeSheet.naturalWidth)return;const frame=dir==='up'?2:dir==='left'||dir==='right'?1:0,cell=overtimeSheet.naturalWidth/3,scale=height/142,size=150*scale,top=feetY-159*scale;context.save();context.imageSmoothingEnabled=true;context.imageSmoothingQuality='high';context.translate(feetX,0);if(dir==='left')context.scale(-1,1);context.drawImage(overtimeSheet,frame*cell,0,cell,overtimeSheet.naturalHeight,-size/2,top,size,size);context.restore();}
function portrait(context,i,showOvertime=true){context.clearRect(0,0,context.canvas.width,context.canvas.height);if(ready){const x=context.canvas.width/2,y=context.canvas.height-3;sprite(context,i,'down',x,y,95,144);if(showOvertime&&isOvertime(people[i]))drawOvertimeFilter(context,'down',x,y,144);}}
...
function withStationStyle(s,draw){ctx.save();if(stationAway(s))ctx.filter='grayscale(1) brightness(1.9) contrast(.85)';draw();ctx.restore();}
...
/** 加班徽章：人還坐在位子上，桌子右側（電腦旁）放一個月亮圖示與「加班中」文字膠囊。 */
function drawOvertimeBadge(s,p){const status=statuses.find(item=>item.id==='overtime'),x=s.x+62,iconY=s.y-70,iconSize=46;drawSymbol(ctx,status.symbol,x,iconY,iconSize);ctx.save();ctx.font='700 12px -apple-system,BlinkMacSystemFont,"PingFang TC","Noto Sans TC",sans-serif';const w=Math.round(ctx.measureText(status.text).width+18),h=20,labelY=iconY+iconSize/2-2;softPanel(ctx,x-w/2,labelY,w,h,{radius:10,fill:'#fff6d6',stroke:'#e2b04a',lineWidth:1,shadow:true});ctx.fillStyle='#6a4a10';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(status.text,x,labelY+h/2+.5);ctx.restore();hits.push({type:'status',i:names.indexOf(p.name),x:x-w/2,y:iconY-iconSize/2,w,h:labelY+h-(iconY-iconSize/2)});}
