...
`furniture.png` 是 1254×1254 的透明高細節家具圖集，目前由 `drawChair` 與 `drawDesk` 分段取樣，以便人物正確穿插在椅子與桌面之間。`icons-refined.png` 是 1536×1024、3×3 的透明圖示圖集；程式依列欄裁切並等比例縮放，避免非方形圖集造成圖示變形。其他圖片保留作參考。
