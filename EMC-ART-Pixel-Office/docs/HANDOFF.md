...
桌子換成使用者重新排版的那組（`assets/furniture-v2.webp`，55 KB，取代 99 KB 的 `furniture-packed.webp`）。

`furnitureRects` 每筆多了第五個值——**螢幕露出桌面的高度**（一般 iMac 桌 37、iMac＋副螢幕 34、Mac mini 桌 25，椅子與空桌 0）。新素材裡桌子本體一律是「桌面 221 + 桌腳 70」，只有螢幕高度因桌子而異，所以 `drawDesk()` 把桌面固定畫 110、桌腳固定畫 35，螢幕再按各自的 upper 等比縮。舊版那串寫死的偏移（`type===2?28:68` 之類）已經移除。`upper` 是 0 時要跳過上段，`drawImage` 不接受高度 0 的來源矩形。

**左下角的空位**（`stations[3]`，沒有 `name`）現在會看時間：台北時間 09:00–18:00 維持原本的顏色與椅子，18:00 到隔天 08:59 連椅子一起轉灰階，但**桌上的電腦留著**（桌子種類仍是 `s.type`，只有配色換成灰階圖集）。判斷在 `stationDimmed()`，它同時涵蓋「有人但離席」與「無人且非上班時段」兩種情況，`furnitureFor()` 與 `drawChair()` 都讀它。
