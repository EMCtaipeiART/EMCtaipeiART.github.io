...
編輯視窗開得大一點有意義：完整版的場景畫在 1536×1024 裡，但**桌子只佔中間那塊（x 424–1112，
約 45% 寬）**，其餘是給人物走動的空地。所以框越寬，人物就畫得越大。2026-09-24 把視窗從
1080×760 開到 1500×960（外層留白 18→10），實測 canvas 642 → 1044 px、場景縮放 0.418 → 0.680、
人物高度 63 → 102 px（**+63%**）。這只動系統這邊的視窗，前台面板的 iframe 完全沒碰。

還想更大的話只剩一條路：在完整版也套 `fitEmbedView()` 那種「只把桌子那塊放到滿」的裁切。
**沒做**，因為那樣人物走出桌子範圍就會看不到（完整版可以用方向鍵／搖桿走動）。
...
> **CSS 不可以再自己決定卡片的位置。** 之前 `@media(max-width:640px)` 用
> `position:fixed!important; top:8px; right:8px` 把卡片釘在右上角，等於整個蓋掉
> `positionPersonCard()` 算好的結果——所以前台那個約 560 px 寬的面板裡，右邊兩位的臉
> 照樣被擋住（2026-09-24 使用者回報）。那條規則現在只留字級與寬度。
> 同一批還修掉一個寫錯的選擇器：`body:not(.embed)`（手機把卡片排到場景下方那條）——
> `embed` class 是加在 `<html>` 上，寫成 `body:not(.embed)` 等於永遠成立，窄框的嵌入版
> 也會被打到，以前是被上面那條 `position:fixed` 蓋過去才沒發現。已改成 `html:not(.embed)`。
>
> 框太小時（約 560×430）一張 252 px 的卡片就佔掉 45% 的寬，**擋到「某個人」是幾何上躲不掉的**；
> 保證的是被點的那一位看得到。點最左邊的 Leona 時卡片只能往右開，這是正常的。
