...
## 自動出勤狀態：電腦開機＝在座、晚上七點後＝加班、關機＝下班（2026-09-21）
...
- 心跳進來、且原本超過 5 分鐘沒有心跳（剛開機／剛醒來，或第一次）：不論原本是什麼狀態，改成「在座」；台北時間 19:00 到隔天 06:00 之間則改成「加班」。
- 電腦持續開著：自動狀態的「在座」會在 19:00 自動切成「加班」。使用者在遊戲裡手動點的狀態（`statusSource:'manual'`）維持到電腦下次關機，不會被蓋掉。
- 超過 5 分鐘沒有心跳（關機或睡眠）：在每次有人讀取狀態（`pixelOfficeState`）時檢查，「在座／加班／廁所」改成「下班」。「出國／公出」不動；使用者在電腦離線之後才手動指定的狀態也尊重。
- 從來沒有心跳的人（沒安裝爬蟲、或共用電腦）完全不會被自動改狀態，維持原本手動。

「加班」（`overtime`，圖示 `moon`）跟其他離席狀態不同：人還坐在位子上工作，所以人物、椅子、電腦照常繪製、也能用方向鍵走動，只是桌邊多一個月亮與「加班中」徽章（`drawOvertimeBadge`）。判斷是否離席請一律用 `isAway(p)`（只有「在座」與「加班」不算離席），不要再直接比 `p.status!=='present'`。加班圖示因為 `icons-v3` 圖集 3×3 已排滿，改由 `moonIconCanvas()` 用程式繪製。狀態按鈕由 5 個變 6 個，改成每列 3 個。
