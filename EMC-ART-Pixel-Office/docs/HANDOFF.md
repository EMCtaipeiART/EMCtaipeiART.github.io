...
- `?embed=1` 是主系統用的精簡模式：`body.embed` 隱藏頁首、編輯工具、名單與說明，但保留 Canvas 的人物點選與人物資料卡。`embedMode` 只擋鍵盤移動，主系統 iframe 必須接受 pointer event，才能讓使用者直接點人物。手機版以 CSS 放大、裁切中央座位區，資料卡固定在 iframe 內；若調整 Canvas 比例，要一起檢查命中座標與資料卡定位。
