...
暖白磁磚地面的像素設計部。六張霧面深藍桌子、螢幕與黑色椅子全部由 Canvas 以同一正面微俯視規格繪製，上下兩排各三張並排接合。椅子、設備、人物、桌子以獨立圖層繪製。座位順序：
...
最新修正：桌中心 X = 548、768、988；角色腳底 Y = 355、695。桌上保留姓名牌，移除頭頂重複姓名；泡泡改為像素圓角框，心情改為太陽、爆炸、水滴、愛心，照片改成可點擊的像素遊戲卡。下班、廁所、出國、公出會隱藏人物、灰階座位並顯示像素狀態牌。
...
`furniture.png` 保留為舊版家具參考，目前畫面不再取樣此圖；桌椅和設備由 `drawChair`、`drawDeskBack`、`drawDeskFront` 統一繪製。其他圖片保留作參考。人物是生成素材；請勿將人物圖集裁切座標誤用於不同尺寸的新圖片。
