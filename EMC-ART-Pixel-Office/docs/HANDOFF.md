...
「加班」（`overtime`，圖示 `moon`）跟其他離席狀態不同：人還坐在位子上工作，所以人物、椅子、電腦照常繪製、也能用方向鍵走動，桌邊會顯示月亮與「加班中」徽章（`drawOvertimeBadge`），人物上另疊上三方向的黑眼圈與鬼火濾鏡（`drawOvertimeFilter`）。濾鏡跟著方向、鏡像、走路浮動與心情旋轉，對齊眼睛。只要實際狀態是加班，或者台北時間 19:00～隔天 06:00 仍是在座，畫面就自動套用；這個視覺有效狀態不寫回後端。本機可加 `?overtime=1` 強制預覽。判斷是否離席請一律用 `isAway(p)`（只有「在座」與「加班」不算離席），不要再直接比 `p.status!=='present'`。加班圖示因為 `icons-v3` 圖集 3×3 已排滿，改由 `moonIconCanvas()` 用程式繪製。狀態按鈕由 5 個變 6 個，改成每列 3 個。
