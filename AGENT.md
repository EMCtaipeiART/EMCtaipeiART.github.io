...
- 影響功能：
  1. **素材**：從兩張來源圖量出桌子的分隔線（桌腳間距 512、分隔線在 x≈568／1085），切出五種素材重新打包。新圖 55 KB，比舊的 `furniture-packed.webp`（99 KB）小 44%；用 WebP q88，不透明區平均色差 1.17（無損版本反而要 202 KB，不划算）。
...
- 已檢查／驗證方式：`node --test backend/test/*.test.mjs` 188/188（新增 3 項：素材與三段切法、左下角時段灰階與椅子同步、未開始紅燈）。本機實機驗證：五種桌子都正確顯示（一般桌、副螢幕桌、Mac mini 桌、空桌、椅子）；離席四人是灰階空桌、在座是藍桌；把時鐘改成 20:00 後左下角整組（桌、椅、Mac mini、黑螢幕）轉灰階且電腦還在，改回 09:08 恢復原色；人物卡三顆燈實測 `rgb(255,90,90)`／`rgb(94,169,255)`／`rgb(240,180,41)`；主系統嵌入畫面六個座位完整置中。
