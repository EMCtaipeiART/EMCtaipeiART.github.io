...
export const DATABASE_HEADERS = [
  ...
  '設計圖資料夾連結', '設計圖檔名關鍵字', '設計圖資料夾清單', 'Gmail信件串ID', 'Gmail寄件帳號'
];
...
export const TABLE_SCHEMAS = {
  ...
  '加權計分標準': {
    primaryKey: null,
    // 「狀態」空字串＝啟用，'下架'＝不再出現在前台表單的階段／項目細節選單，但**計分規則仍然保留**
  ...
  '修改統計表': {
    primaryKey: null,
    headers: ['案件編號', '修改次數', '建立日期', '修改日期', '修改內容', '修改內容連結', '修改人', '確認修正日', '圖片連結', '圖片來源', '圖片更新時間', '待修改圖片']
  },
  '補充資料連結': {
    primaryKey: '案件編號',
  ...
  '設定': {
    ...
    headers: [
      ...
      '音樂起始秒數', '技能', '技能表單設定', '對話框', '回信範本設定', '預設回信範本', '簽名檔清單', '預設簽名檔', '新專案輪值', '設計師顯示', '篩選年份', '篩選月份', '篩選狀態',
  ...
  reels: {
    ...
    // 「已讀」跟「按讚」「倒讚」同樣是逗號分隔的名字清單——同一個人只會出現一次（第一次看到
    // 就記錄，之後重複瀏覽不會重複計算），只用來給後台看「有誰看過」，不是每次瀏覽都累加的次數。
    headers: ['名字', '限時動態連結', '保留期限', '到期時間', '按讚', '倒讚', '留言', '狀態', '已讀']
  },
  bug_report: {
    primaryKey: null,
