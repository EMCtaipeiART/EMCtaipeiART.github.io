...
`加權` 依 JSON 的 `加權計分標準` 資料表計算：數量乘以所有一般項目細節的權重總和，再加上急件權重（預設 3 分，急件不乘數量）。管理者可在資料庫後台直接編輯 `權重`；Node API 修改規則後會在同一筆交易內重算全部 `database` 案件。項目細節空白時不計分，欄位保持空白。可執行 `npm run weights:recalculate` 重新計算既有 JSON 案件。
