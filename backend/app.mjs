...
const DEFAULT_DESIGNER_NAMES = Object.freeze(Object.values(PROJECT_GROUPS).flatMap(group => group.designers));
function isDesignerSettingsRow(row = {}) {
...
function designerRowsForGroup(snapshot, group) {
  return snapshot.tables['設定'].rows.filter(row => isDesignerSettingsRow(row) && text(row['組別']) === group)
    .sort((left, right) => (Number(left['新專案輪值']) || 99) - (Number(right['新專案輪值']) || 99) || text(left['名字']).localeCompare(text(right['名字']), 'zh-Hant'));
}
function normalizedSkillMappings(value) {
  let rows = value;
  if (!Array.isArray(rows)) { try { rows = JSON.parse(text(value)); } catch { rows = []; } }
  ...
  return async function handleAction(action, payload = {}, context = {}) {
    ...
    if (action === 'markSystemAnnouncementRead') {
      ...
      return database.transaction(draft => {
        ...
        if (!records.some(record => record.account === account)) {
          const profile = settingsRow(draft, account || session.user);
          records.push({
            account,
            name: text(profile?.['顯示名'] || profile?.['名字'] || session.user || account),
            readAt: nowTaipei()
          });
          row['已讀紀錄'] = JSON.stringify(records);
        }
        return { ok: true, action, version, account, readCount: records.length, readRecords: records };
      }, 'mark system announcement read');
    }
    ...
    if (action === 'listDesignerProfiles') {
      const profiles = snapshot.tables['設定'].rows.filter(isDesignerSettingsRow).map(row => ({
    ...
    if (action === 'adminDesignerSave') {
      ...
      const account = canonicalAccount(payload.account), profile = payload.profile || payload.settings || {}, expected = payload.expectedSettingsRow || {};
      ...
      for (const [key, label] of [['avatar', '頭像連結'], ['poster', '頭像大圖連結'], ['musicUrl', '分享音樂']]) if (text(profile[key]) && !isHttpUrl(profile[key])) throw new Error(`「${label}」必須是 http 或 https 網址`);
      if (text(profile.quote).length > 120) throw new Error('對話框不得超過 120 個字');
      return database.transaction(draft => {
        const table = draft.tables['設定'], row = table.rows.find(item => canonicalAccount(item['帳號']) === account);
        ...
        Object.assign(row, {
          '組別': group, '設計師顯示': 'v', '頭像連結': text(profile.avatar), '頭像大圖連結': text(profile.poster),
          '分享音樂': text(profile.musicUrl), '音樂起始秒數': String(Math.max(0, Math.floor(Number(profile.musicStartAt) || 0))),
    ...
    if (action === 'adminDesignerRemove') {
      requireCapability(snapshot, payload, 'database.manage');
      const account = canonicalAccount(payload.account), expected = payload.expectedSettingsRow || {};
      if (!account) throw new Error('缺少要移除的設計師帳號');
      return database.transaction(draft => {
        const table = draft.tables['設定'], row = table.rows.find(item => canonicalAccount(item['帳號']) === account);
        if (!row || !isDesignerSettingsRow(row)) throw new Error('找不到啟用中的設計師');
        if (Object.keys(expected).length && rowsDiffer(table.headers, expected, row)) throw new Error('設計師資料已被其他人更新，請重新讀取後再操作');
        const group = text(row['組別']);row['設計師顯示'] = 'x';row['新專案輪值'] = '';
        designerRowsForGroup(draft, group).forEach((item, index) => { item['新專案輪值'] = String(index + 1); });
        return { ok: true, action, account, name: text(row['名字']), changedTables: ['設定'] };
      }, 'admin designer remove');
    }
    ...
    if (action === 'uploadDesignerImage') {
      ...
      if (!snapshot.tables['設定'].rows.some(row => isDesignerSettingsRow(row) && text(row['名字']) === name)) throw new Error('找不到設計師');
    ...
    if (action === 'createFlatProject') {
      ...
      const groupEntry = ['平面', '影音'].map(group => [group, { designers: designerRowsForGroup(snapshot, group).map(row => text(row['名字'])), type: group }]).find(([, config]) => config.designers.includes(expected));
